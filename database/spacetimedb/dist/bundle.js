...
const spacetimedb = schema({
	...
	published: table({ public: true }, { owner_key: t.string().primaryKey() })
});
...
const publish = spacetimedb.procedure({
	...
	key: t.string(),
	del: t.bool()
}, t.enum("publishResult", ["success", "err"]), (ctx, { owner, passhash, key, del }) => ctx.withTx((c) => {
	if (!checkAuth(c, owner, passhash)) return err;
	let owner_key = mkkey(owner, key);
