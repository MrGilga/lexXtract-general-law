...
// model/pattern.ts
var ANY, isObject, isRefPattern, isDefsPattern, hasStringId, isKeywordKey, literalKey, propKey, normalProps, eachChild, toSchema, format, rootBase, stripFragment, fragmentOf, decodePointer, resolveUri, scopeBase, hasId, deref, resolve, validate, fill, SchemaPattern, TaxonomyPattern;
var init_pattern = __esm({
  "model/pattern.ts"() {
    "use strict";
    init_json();
    ANY = { "$any": true };
    isObject = (pattern) => typeof pattern == "object" && pattern != null && !Array.isArray(pattern);
    isRefPattern = (pattern) => isObject(pattern) && "$ref" in pattern && typeof pattern.$ref == "string";
    isDefsPattern = (pattern) => isObject(pattern) && "$defs" in pattern && isObject(pattern.$defs) && "pattern" in pattern;
    hasStringId = (pattern) => isObject(pattern) && typeof pattern.$id == "string";
    isKeywordKey = (key) => key.startsWith("$") && !key.startsWith("$$");
    literalKey = (key) => key.startsWith("$$") ? key.slice(1) : key;
    propKey = (key) => {
      key = literalKey(key);
      return { key: key.endsWith("?") ? key.slice(0, -1) : key, optional: key.endsWith("?") };
    };
    normalProps = (pattern) => Object.entries(pattern).filter(([k]) => k != "[key:string]" && !isKeywordKey(k)).map(([k, pattern2]) => ({ ...propKey(k), pattern: pattern2 }));
    eachChild = (pattern, fn) => {
    ...
    toSchema = (pattern) => {
      const _toSchema = (pattern2) => {
        if (typeof pattern2 == "object" && pattern2 != null && "$any" in pattern2) return {};
        if (pattern2 == String) return { type: "string" };
        if (pattern2 == Number) return { type: "number" };
        if (pattern2 == Boolean) return { type: "boolean" };
        if (typeof pattern2 == "string" || typeof pattern2 == "number" || typeof pattern2 == "boolean" || pattern2 === null) return { const: pattern2 };
        if (pattern2 instanceof Array && pattern2.length == 1) return { type: "array", items: _toSchema(pattern2[0]) };
        if (pattern2 instanceof Array) return { anyOf: pattern2.map(_toSchema) };
        if (isObject(pattern2)) {
          if (pattern2 == null) return { type: "null" };
          if ("$const" in pattern2) return { const: pattern2["$const"] };
          if (isRefPattern(pattern2)) return pattern2;
          let props = {};
          let required = [];
          ...
          if (hasStringId(pattern2)) res.$id = pattern2.$id;
    ...
    rootBase = "pattern:/";
    stripFragment = (uri) => uri.split("#")[0] || "";
    fragmentOf = (uri) => uri.includes("#") ? "#" + uri.split("#").slice(1).join("#") : "";
    decodePointer = (part) => part.replaceAll("~1", "/").replaceAll("~0", "~");
    resolveUri = (base, ref) => {
      if (!base) base = rootBase;
      return new URL(ref, base).toString();
    };
    scopeBase = (base, pattern) => {
      if (hasStringId(pattern)) {
        return resolveUri(base, pattern.$id);
      }
      return base || rootBase;
    };
    hasId = (pattern) => hasStringId(pattern);
    deref = (target, ref, err) => {
...
${JSON.stringify(pattern, null, 2)}`);
      ...
      if (found == void 0) throw err;
      return found;
    };
    validate = (pattern, data) => {
      let go = (p, d, path2, base = "") => {
        let nextBase = scopeBase(base, p);
        const raise = (msg) => {
          throw new Error(`Validation error at ${path2.join(".")}:
${format(p, pattern)}
vs data: ${stringify(d)}
${msg}`);
        };
        const assert = (condition, msg) => {
          if (!condition) raise(msg || "assertion failed");
        };
        ...
        if (p instanceof Array && p.length == 1) {
          ...
          d.forEach((x, i) => go(p[0], x, [...path2, String(i)], nextBase));
          return;
        }
        ...
        if (isObject(p)) {
          ...
          if (isRefPattern(p)) {
            ...
            return go(ref, d, path2, resolveUri(base, p.$ref));
          }
          if (isDefsPattern(p)) return go(p.pattern, d, path2, nextBase);
          ...
          Object.entries(d).forEach(([k, v]) => {
            if (k in props) return go(props[k].pattern, v, [...path2, k], nextBase);
            assert("[key:string]" in p, `unexpected property ${k}`);
            return go(p["[key:string]"], v, [...path2, k], nextBase);
          });
...
var import__, configure, stringify2;
var init_wrapper = __esm({
  "node_modules/safe-stable-stringify/esm/wrapper.js"() {
    import__ = __toESM(require_safe_stable_stringify(), 1);
    configure = import__.default.configure;
    stringify2 = import__.default;
  }
});
...
function coerceParams(params5) {
  return Object.fromEntries(
    Object.entries(params5).map(([n, c]) => [
      n,
...
function procedureSchema(name, params5, returnType5) {
  return {
    name,
    accessorName: toCamelCase(name),
    params: coerceParams(params5),
    returnType: returnType5
  };
}
var import_base64_js, TimeDuration, Timestamp, Uuid, BinaryReader, ArrayBufferPrototypeTransfer, ResizableBuffer, BinaryWriter, hasOwn, Identity, SERIALIZERS, DESERIALIZERS, AlgebraicType, primitiveSerializers, serializeUint8Array, primitiveDeserializers, deserializeUint8Array, primitiveSizes, fixedSizePrimitives, isFixedSizeProduct, productSize, primitiveJSName, specialProductDeserializers, unitDeserializer, getElementInitializer, ProductType, SumType, ConnectionId, SenderError, InternalError, ScheduleAt, Interval, Time, schedule_at_default, Option, Result, QueryBrand, isRowTypedQuery, SemijoinImpl, FromBuilder, TableRefImpl, ColumnExpression, BooleanExpr, TypeBuilder, U8Builder, U16Builder, U32Builder, U64Builder, U128Builder, U256Builder, I8Builder, I16Builder, I32Builder, I64Builder, I128Builder, I256Builder, F32Builder, F64Builder, BoolBuilder, StringBuilder, ArrayBuilder, ByteArrayBuilder, OptionBuilder, ProductBuilder, ResultBuilder, UnitBuilder, RowBuilder, SumBuilderImpl, SumBuilder, SimpleSumBuilderImpl, ScheduleAtBuilder, IdentityBuilder, ConnectionIdBuilder, TimestampBuilder, TimeDurationBuilder, UuidBuilder, defaultMetadata, ColumnBuilder, U8ColumnBuilder, U16ColumnBuilder, U32ColumnBuilder, U64ColumnBuilder, U128ColumnBuilder, U256ColumnBuilder, I8ColumnBuilder, I16ColumnBuilder, I32ColumnBuilder, I64ColumnBuilder, I128ColumnBuilder, I256ColumnBuilder, F32ColumnBuilder, F64ColumnBuilder, BoolColumnBuilder, StringColumnBuilder, ArrayColumnBuilder, ByteArrayColumnBuilder, OptionColumnBuilder, ResultColumnBuilder, ProductColumnBuilder, SumColumnBuilder, SimpleSumColumnBuilder, ScheduleAtColumnBuilder, IdentityColumnBuilder, ConnectionIdColumnBuilder, TimestampColumnBuilder, TimeDurationColumnBuilder, UuidColumnBuilder, RefBuilder, enumImpl, t, BsatnRowList, CallProcedure, CallReducer, ClientMessage, EventTableRows, InitialConnection, OneOffQuery, OneOffQueryResult, PersistentTableRows, ProcedureResult, ProcedureStatus, QueryRows, QuerySetId, QuerySetUpdate, ReducerOk, ReducerOutcome, ReducerResult, RowSizeHint, ServerMessage, SingleTableRows, Subscribe, SubscribeApplied, SubscriptionError, TableUpdate, TableUpdateRows, TransactionUpdate, Unsubscribe, UnsubscribeApplied, UnsubscribeFlags, EventEmitter, LogLevelIdentifierIcon, LogStyle, LogTextStyle, LogLevelRank, globalLogLevel, shouldLog, resolveLazy, toHex, ARRAY_TRUNCATION_THRESHOLD, ARRAY_PREVIEW_COUNT, SENSITIVE_KEYS, stringify3, stdbLogger, scalarCompare, TableCacheImpl, TableMap, ClientCache, SemanticVersion, _MINIMUM_CLI_VERSION, WebsocketDecompressAdapter, DbConnectionBuilder, INTERNAL_REMOTE_MODULE, SubscriptionBuilderImpl, SubscriptionManager, SubscriptionHandleImpl, DbConnectionImpl, ModuleContext, Tables, AlgebraicType2, CaseConversionPolicy, ExplicitNameEntry, ExplicitNames, FunctionVisibility, HttpHeaderPair, HttpHeaders, HttpMethod, HttpVersion, IndexType, Lifecycle, MiscModuleExport, NameMapping, ProductType2, ProductTypeElement, RawColumnDefV8, RawColumnDefaultValueV10, RawColumnDefaultValueV9, RawConstraintDataV9, RawConstraintDefV10, RawConstraintDefV8, RawConstraintDefV9, RawIndexAlgorithm, RawIndexDefV10, RawIndexDefV8, RawIndexDefV9, RawLifeCycleReducerDefV10, RawMiscModuleExportV9, RawModuleDefV10, RawModuleDefV10Section, RawModuleDefV8, RawModuleDefV9, RawProcedureDefV10, RawProcedureDefV9, RawReducerDefV10, RawReducerDefV9, RawRowLevelSecurityDefV9, RawScheduleDefV10, RawScheduleDefV9, RawScopedTypeNameV10, RawScopedTypeNameV9, RawSequenceDefV10, RawSequenceDefV8, RawSequenceDefV9, RawTableDefV10, RawTableDefV8, RawTableDefV9, RawTypeDefV10, RawTypeDefV9, RawUniqueConstraintDataV9, RawViewDefV10, RawViewDefV9, ReducerDef, SumType2, SumTypeVariant, TableAccess, TableDesc, TableType, TypeAlias, Typespace, Reducers;
var init_dist = __esm({
  "node_modules/spacetimedb/dist/index.mjs"() {
    ...
    ArrayBufferPrototypeTransfer = ArrayBuffer.prototype.transfer ?? function(newByteLength) {
    ...
    DESERIALIZERS = /* @__PURE__ */ new Map();
    AlgebraicType = {
    ...
    serializeUint8Array = bindCall(BinaryWriter.prototype.writeUInt8Array);
    ...
    deserializeUint8Array = bindCall(BinaryReader.prototype.readUInt8Array);
    primitiveSizes = {
      Bool: 1,
      I8: 1,
      U8: 1,
    ...
    fixedSizePrimitives = new Set(Object.keys(primitiveSizes));
    isFixedSizeProduct = (ty) => ty.elements.every(
      ({ algebraicType }) => fixedSizePrimitives.has(algebraicType.tag)
    );
    productSize = (ty) => ty.elements.reduce(
...
${ty.variants.map(
    ...
    QueryBrand = /* @__PURE__ */ Symbol("QueryBrand");
    isRowTypedQuery = (val) => !!val && typeof val === "object" && QueryBrand in val;
    SemijoinImpl = class _SemijoinImpl {
    ...
    FromBuilder = class _FromBuilder {
    ...
    BooleanExpr = class _BooleanExpr {
      constructor(data) {
        this.data = data;
      }
      and(other) {
    ...
    U8Builder = class extends TypeBuilder {
      constructor() {
    ...
    U16Builder = class extends TypeBuilder {
      constructor() {
    ...
    U32Builder = class extends TypeBuilder {
      constructor() {
    ...
    U64Builder = class extends TypeBuilder {
      constructor() {
    ...
    U128Builder = class extends TypeBuilder {
      constructor() {
    ...
    U256Builder = class extends TypeBuilder {
      constructor() {
    ...
    I8Builder = class extends TypeBuilder {
      constructor() {
    ...
    I16Builder = class extends TypeBuilder {
      constructor() {
    ...
    I32Builder = class extends TypeBuilder {
      constructor() {
    ...
    I64Builder = class extends TypeBuilder {
      constructor() {
    ...
    I128Builder = class extends TypeBuilder {
      constructor() {
    ...
    I256Builder = class extends TypeBuilder {
      constructor() {
    ...
    F32Builder = class extends TypeBuilder {
      constructor() {
    ...
    F64Builder = class extends TypeBuilder {
      constructor() {
    ...
    BoolBuilder = class extends TypeBuilder {
      constructor() {
    ...
    StringBuilder = class extends TypeBuilder {
      constructor() {
    ...
    ArrayBuilder = class extends TypeBuilder {
      element;
      constructor(element) {
    ...
    ByteArrayBuilder = class extends TypeBuilder {
      constructor() {
    ...
    OptionBuilder = class extends TypeBuilder {
      value;
      constructor(value) {
    ...
    ProductBuilder = class extends TypeBuilder {
      typeName;
      elements;
    ...
    ResultBuilder = class extends TypeBuilder {
      ok;
      err;
      constructor(ok, err) {
    ...
    UnitBuilder = class extends TypeBuilder {
      constructor() {
        super({ tag: "Product", value: { elements: [] } });
      }
    };
    RowBuilder = class extends TypeBuilder {
      row;
      typeName;
    ...
    SumBuilderImpl = class extends TypeBuilder {
      variants;
      typeName;
    ...
    SumBuilder = SumBuilderImpl;
    SimpleSumBuilderImpl = class extends SumBuilderImpl {
      index(algorithm = "btree") {
    ...
    ScheduleAtBuilder = class extends TypeBuilder {
      constructor() {
    ...
    IdentityBuilder = class extends TypeBuilder {
      constructor() {
    ...
    ConnectionIdBuilder = class extends TypeBuilder {
      constructor() {
    ...
    TimestampBuilder = class extends TypeBuilder {
      constructor() {
    ...
    TimeDurationBuilder = class extends TypeBuilder {
      constructor() {
    ...
    UuidBuilder = class extends TypeBuilder {
      constructor() {
    ...
    U8ColumnBuilder = class _U8ColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    U16ColumnBuilder = class _U16ColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    U32ColumnBuilder = class _U32ColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    U64ColumnBuilder = class _U64ColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    U128ColumnBuilder = class _U128ColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    U256ColumnBuilder = class _U256ColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    I8ColumnBuilder = class _I8ColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    I16ColumnBuilder = class _I16ColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    I32ColumnBuilder = class _I32ColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    I64ColumnBuilder = class _I64ColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    I128ColumnBuilder = class _I128ColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    I256ColumnBuilder = class _I256ColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    F32ColumnBuilder = class _F32ColumnBuilder extends ColumnBuilder {
      default(value) {
    ...
    F64ColumnBuilder = class _F64ColumnBuilder extends ColumnBuilder {
      default(value) {
    ...
    BoolColumnBuilder = class _BoolColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    StringColumnBuilder = class _StringColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    ArrayColumnBuilder = class _ArrayColumnBuilder extends ColumnBuilder {
      default(value) {
    ...
    ByteArrayColumnBuilder = class _ByteArrayColumnBuilder extends ColumnBuilder {
    ...
    OptionColumnBuilder = class _OptionColumnBuilder extends ColumnBuilder {
      default(value) {
    ...
    ResultColumnBuilder = class _ResultColumnBuilder extends ColumnBuilder {
      constructor(typeBuilder, metadata) {
    ...
    ProductColumnBuilder = class _ProductColumnBuilder extends ColumnBuilder {
      default(value) {
    ...
    SumColumnBuilder = class _SumColumnBuilder extends ColumnBuilder {
      default(value) {
    ...
    SimpleSumColumnBuilder = class _SimpleSumColumnBuilder extends SumColumnBuilder {
      index(algorithm = "btree") {
    ...
    ScheduleAtColumnBuilder = class _ScheduleAtColumnBuilder extends ColumnBuilder {
      default(value) {
    ...
    IdentityColumnBuilder = class _IdentityColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    ConnectionIdColumnBuilder = class _ConnectionIdColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    TimestampColumnBuilder = class _TimestampColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    TimeDurationColumnBuilder = class _TimeDurationColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    UuidColumnBuilder = class _UuidColumnBuilder extends ColumnBuilder {
      index(algorithm = "btree") {
    ...
    RefBuilder = class extends TypeBuilder {
      ref;
    ...
    BsatnRowList = t.object("BsatnRowList", {
      get sizeHint() {
        return RowSizeHint;
      },
      rowsData: t.byteArray()
    });
    CallProcedure = t.object("CallProcedure", {
      requestId: t.u32(),
      flags: t.u8(),
      procedure: t.string(),
      args: t.byteArray()
    });
    CallReducer = t.object("CallReducer", {
      requestId: t.u32(),
      flags: t.u8(),
      reducer: t.string(),
      args: t.byteArray()
    });
    ClientMessage = t.enum("ClientMessage", {
      get Subscribe() {
        return Subscribe;
      },
      get Unsubscribe() {
    ...
    EventTableRows = t.object("EventTableRows", {
      get events() {
        return BsatnRowList;
      }
    });
    InitialConnection = t.object("InitialConnection", {
      identity: t.identity(),
      connectionId: t.connectionId(),
      token: t.string()
    });
    OneOffQuery = t.object("OneOffQuery", {
      requestId: t.u32(),
      queryString: t.string()
    });
    OneOffQueryResult = t.object("OneOffQueryResult", {
      requestId: t.u32(),
      get result() {
        return t.result(QueryRows, t.string());
      }
    });
    PersistentTableRows = t.object("PersistentTableRows", {
      get inserts() {
        return BsatnRowList;
      },
      get deletes() {
    ...
    ProcedureResult = t.object("ProcedureResult", {
      get status() {
    ...
    ProcedureStatus = t.enum("ProcedureStatus", {
      Returned: t.byteArray(),
      InternalError: t.string()
    });
    QueryRows = t.object("QueryRows", {
      get tables() {
        return t.array(SingleTableRows);
      }
    });
    QuerySetId = t.object("QuerySetId", {
      id: t.u32()
    });
    QuerySetUpdate = t.object("QuerySetUpdate", {
      get querySetId() {
        return QuerySetId;
      },
      get tables() {
    ...
    ReducerOk = t.object("ReducerOk", {
      retValue: t.byteArray(),
      get transactionUpdate() {
        return TransactionUpdate;
      }
    });
    ReducerOutcome = t.enum("ReducerOutcome", {
      get Ok() {
        return ReducerOk;
      },
      OkEmpty: t.unit(),
    ...
    ReducerResult = t.object("ReducerResult", {
      requestId: t.u32(),
      timestamp: t.timestamp(),
      get result() {
    ...
    RowSizeHint = t.enum("RowSizeHint", {
      FixedSize: t.u16(),
      RowOffsets: t.array(t.u64())
    });
    ServerMessage = t.enum("ServerMessage", {
    ...
    SingleTableRows = t.object("SingleTableRows", {
      table: t.string(),
      get rows() {
        return BsatnRowList;
      }
    });
    Subscribe = t.object("Subscribe", {
      requestId: t.u32(),
      get querySetId() {
        return QuerySetId;
      },
    ...
    SubscribeApplied = t.object("SubscribeApplied", {
      requestId: t.u32(),
      get querySetId() {
        return QuerySetId;
      },
    ...
    SubscriptionError = t.object("SubscriptionError", {
    ...
    TableUpdate = t.object("TableUpdate", {
      tableName: t.string(),
      get rows() {
        return t.array(TableUpdateRows);
      }
    });
    TableUpdateRows = t.enum("TableUpdateRows", {
      get PersistentTable() {
    ...
    TransactionUpdate = t.object("TransactionUpdate", {
      get querySets() {
        return t.array(QuerySetUpdate);
      }
    });
    Unsubscribe = t.object("Unsubscribe", {
      requestId: t.u32(),
      get querySetId() {
        return QuerySetId;
      },
    ...
    UnsubscribeApplied = t.object("UnsubscribeApplied", {
      requestId: t.u32(),
      get querySetId() {
        return QuerySetId;
      },
    ...
    UnsubscribeFlags = t.enum("UnsubscribeFlags", {
      Default: t.unit(),
    ...
    globalLogLevel = "info";
    shouldLog = (level) => LogLevelRank[level] <= LogLevelRank[globalLogLevel];
    resolveLazy = (v) => typeof v === "function" ? v() : v;
    toHex = (bytes) => Array.from(bytes).map((b) => b.toString(16).padStart(2, "0")).join("");
    ARRAY_TRUNCATION_THRESHOLD = 25;
    ARRAY_PREVIEW_COUNT = 10;
    ...
    stringify3 = (value) => stringify2(value, (key, current) => {
      ...
      if (Array.isArray(current) && current.length >= ARRAY_TRUNCATION_THRESHOLD) {
        const head = stringify2(current.slice(0, ARRAY_PREVIEW_COUNT));
        return `Array(len=${current.length}, head=${head ?? "[]"})`;
      }
      return current;
    });
    stdbLogger = (level, message, ...args) => {
      if (!shouldLog(level)) {
        return;
      }
    ...
    INTERNAL_REMOTE_MODULE = /* @__PURE__ */ Symbol("INTERNAL_REMOTE_MODULE");
    ...
    SubscriptionHandleImpl = class {
      constructor(db2, querySql, onApplied, onError) {
        this.db = db2;
        this.#emitter.on(
          "applied",
          (ctx) => {
    ...
    DbConnectionImpl = class {
      ...
      #makeReducers(def) {
        ...
        for (const reducer of def.reducers) {
          ...
          out[key] = (params5) => {
            writer.clear();
            serializeArgs(writer, params5);
            const argsBuffer = writer.getBuffer();
            return this.callReducer(reducerName, argsBuffer, params5);
          };
        }
        return out;
      }
      ...
      #sendMessage(message) {
        ...
        if (this.ws && this.isActive) {
          ...
          stdbLogger(
            "trace",
            () => `Sending message to server: ${stringify3(message)}`
          );
          this.ws.send(encoded);
        } else {
          stdbLogger(
            "trace",
            () => `Queuing message to server: ${stringify3(message)}`
          );
      ...
      async #processMessage(data) {
        ...
        stdbLogger(
          "trace",
          () => `Processing server message: ${stringify3(serverMessage)}`
        );
       ...
       */
      callReducerWithParams(reducerName, _paramsType, params5) {
        const writer = new BinaryWriter(1024);
        this.#reducerArgsSerializers[reducerName].serialize(writer, params5);
        const argsBuffer = writer.getBuffer();
        return this.callReducer(reducerName, argsBuffer, params5);
      }
      /**
       ...
       * @param params The arguments to pass to the reducer
       */
      callProcedureWithParams(procedureName, _paramsType, params5, _returnType) {
        const writer = new BinaryWriter(1024);
        const { serializeArgs, deserializeReturn } = this.#procedureSerializers[procedureName];
        serializeArgs(writer, params5);
        const argsBuffer = writer.getBuffer();
    ...
    AlgebraicType2 = t.enum("AlgebraicType", {
      Ref: t.u32(),
      get Sum() {
        return SumType2;
      },
    ...
    CaseConversionPolicy = t.enum("CaseConversionPolicy", {
      None: t.unit(),
      SnakeCase: t.unit()
    });
    ExplicitNameEntry = t.enum("ExplicitNameEntry", {
      get Table() {
        return NameMapping;
      },
      get Function() {
    ...
    ExplicitNames = t.object("ExplicitNames", {
      get entries() {
        return t.array(ExplicitNameEntry);
      }
    });
    FunctionVisibility = t.enum("FunctionVisibility", {
      Private: t.unit(),
      ClientCallable: t.unit()
    });
    HttpHeaderPair = t.object("HttpHeaderPair", {
      name: t.string(),
      value: t.byteArray()
    });
    HttpHeaders = t.object("HttpHeaders", {
      get entries() {
        return t.array(HttpHeaderPair);
      }
    });
    HttpMethod = t.enum("HttpMethod", {
      Get: t.unit(),
      Head: t.unit(),
      Post: t.unit(),
      Put: t.unit(),
    ...
    HttpVersion = t.enum("HttpVersion", {
      Http09: t.unit(),
      Http10: t.unit(),
      Http11: t.unit(),
      Http2: t.unit(),
    ...
    IndexType = t.enum("IndexType", {
      BTree: t.unit(),
      Hash: t.unit()
    });
    Lifecycle = t.enum("Lifecycle", {
      Init: t.unit(),
      OnConnect: t.unit(),
      OnDisconnect: t.unit()
    });
    MiscModuleExport = t.enum("MiscModuleExport", {
      get TypeAlias() {
        return TypeAlias;
      }
    });
    NameMapping = t.object("NameMapping", {
      sourceName: t.string(),
      canonicalName: t.string()
    });
    ProductType2 = t.object("ProductType", {
      get elements() {
        return t.array(ProductTypeElement);
      }
    });
    ProductTypeElement = t.object("ProductTypeElement", {
      name: t.option(t.string()),
      get algebraicType() {
        return AlgebraicType2;
      }
    });
    RawColumnDefV8 = t.object("RawColumnDefV8", {
      colName: t.string(),
      get colType() {
        return AlgebraicType2;
      }
    ...
    RawColumnDefaultValueV9 = t.object("RawColumnDefaultValueV9", {
      table: t.string(),
      colId: t.u16(),
      value: t.byteArray()
    });
    RawConstraintDataV9 = t.enum("RawConstraintDataV9", {
      get Unique() {
        return RawUniqueConstraintDataV9;
      }
    });
    RawConstraintDefV10 = t.object("RawConstraintDefV10", {
      sourceName: t.option(t.string()),
      get data() {
        return RawConstraintDataV9;
      }
    });
    RawConstraintDefV8 = t.object("RawConstraintDefV8", {
      constraintName: t.string(),
      constraints: t.u8(),
      columns: t.array(t.u16())
    });
    RawConstraintDefV9 = t.object("RawConstraintDefV9", {
      name: t.option(t.string()),
      get data() {
        return RawConstraintDataV9;
      }
    });
    RawIndexAlgorithm = t.enum("RawIndexAlgorithm", {
      BTree: t.array(t.u16()),
      Hash: t.array(t.u16()),
      Direct: t.u16()
    });
    RawIndexDefV10 = t.object("RawIndexDefV10", {
      sourceName: t.option(t.string()),
    ...
    RawIndexDefV8 = t.object("RawIndexDefV8", {
      indexName: t.string(),
      isUnique: t.bool(),
      get indexType() {
        return IndexType;
    ...
    RawIndexDefV9 = t.object("RawIndexDefV9", {
      name: t.option(t.string()),
      accessorName: t.option(t.string()),
      get algorithm() {
        return RawIndexAlgorithm;
    ...
    RawLifeCycleReducerDefV10 = t.object(
    ...
    RawMiscModuleExportV9 = t.enum("RawMiscModuleExportV9", {
    ...
    RawModuleDefV10 = t.object("RawModuleDefV10", {
      get sections() {
        return t.array(RawModuleDefV10Section);
      }
    });
    RawModuleDefV10Section = t.enum("RawModuleDefV10Section", {
      get Typespace() {
        return Typespace;
      },
      get Types() {
    ...
    RawModuleDefV8 = t.object("RawModuleDefV8", {
      get typespace() {
        return Typespace;
      },
      get tables() {
    ...
    RawModuleDefV9 = t.object("RawModuleDefV9", {
      get typespace() {
        return Typespace;
      },
      get tables() {
    ...
    RawProcedureDefV10 = t.object("RawProcedureDefV10", {
      sourceName: t.string(),
      get params() {
        return ProductType2;
      },
    ...
    RawProcedureDefV9 = t.object("RawProcedureDefV9", {
      name: t.string(),
      get params() {
        return ProductType2;
      },
    ...
    RawReducerDefV10 = t.object("RawReducerDefV10", {
      sourceName: t.string(),
      get params() {
        return ProductType2;
      },
    ...
    RawReducerDefV9 = t.object("RawReducerDefV9", {
      name: t.string(),
      get params() {
        return ProductType2;
      },
    ...
    RawRowLevelSecurityDefV9 = t.object("RawRowLevelSecurityDefV9", {
      sql: t.string()
    });
    RawScheduleDefV10 = t.object("RawScheduleDefV10", {
      sourceName: t.option(t.string()),
      tableName: t.string(),
      scheduleAtCol: t.u16(),
      functionName: t.string()
    });
    RawScheduleDefV9 = t.object("RawScheduleDefV9", {
      name: t.option(t.string()),
      reducerName: t.string(),
      scheduledAtColumn: t.u16()
    });
    RawScopedTypeNameV10 = t.object("RawScopedTypeNameV10", {
      scope: t.array(t.string()),
      sourceName: t.string()
    });
    RawScopedTypeNameV9 = t.object("RawScopedTypeNameV9", {
      scope: t.array(t.string()),
      name: t.string()
    });
    RawSequenceDefV10 = t.object("RawSequenceDefV10", {
      sourceName: t.option(t.string()),
      column: t.u16(),
    ...
    RawSequenceDefV8 = t.object("RawSequenceDefV8", {
    ...
    RawSequenceDefV9 = t.object("RawSequenceDefV9", {
      name: t.option(t.string()),
      column: t.u16(),
      start: t.option(t.i128()),
      minValue: t.option(t.i128()),
    ...
    RawTableDefV10 = t.object("RawTableDefV10", {
      sourceName: t.string(),
    ...
    RawTableDefV8 = t.object("RawTableDefV8", {
      tableName: t.string(),
      get columns() {
    ...
    RawTableDefV9 = t.object("RawTableDefV9", {
      name: t.string(),
      productTypeRef: t.u32(),
      primaryKey: t.array(t.u16()),
      get indexes() {
    ...
    RawTypeDefV10 = t.object("RawTypeDefV10", {
      get sourceName() {
    ...
    RawTypeDefV9 = t.object("RawTypeDefV9", {
      get name() {
    ...
    RawUniqueConstraintDataV9 = t.object(
      "RawUniqueConstraintDataV9",
      {
        columns: t.array(t.u16())
      }
    );
    RawViewDefV10 = t.object("RawViewDefV10", {
      sourceName: t.string(),
      index: t.u32(),
      isPublic: t.bool(),
      isAnonymous: t.bool(),
    ...
    RawViewDefV9 = t.object("RawViewDefV9", {
      name: t.string(),
      index: t.u32(),
      isPublic: t.bool(),
      isAnonymous: t.bool(),
    ...
    ReducerDef = t.object("ReducerDef", {
      name: t.string(),
      get args() {
        return t.array(ProductTypeElement);
      }
    });
    SumType2 = t.object("SumType", {
      get variants() {
        return t.array(SumTypeVariant);
      }
    });
    SumTypeVariant = t.object("SumTypeVariant", {
      name: t.option(t.string()),
      get algebraicType() {
        return AlgebraicType2;
      }
    });
    TableAccess = t.enum("TableAccess", {
      Public: t.unit(),
      Private: t.unit()
    });
    TableDesc = t.object("TableDesc", {
      get schema() {
        return RawTableDefV8;
      },
      data: t.u32()
    });
    TableType = t.enum("TableType", {
      System: t.unit(),
      User: t.unit()
    });
    TypeAlias = t.object("TypeAlias", {
      name: t.string(),
      ty: t.u32()
    });
    Typespace = t.object("Typespace", {
      get types() {
...
var ChangePasswordResult, Person, PublishResult, Published, SetResult, SignupResult, Storage;
var init_types = __esm({
  "model/module_bindings/types.ts"() {
    "use strict";
    init_dist();
    ChangePasswordResult = t.enum("ChangePasswordResult", {
      Success: t.unit(),
      Err: t.unit()
    });
    Person = t.object("Person", {
      userid: t.string(),
      passhash: t.string()
    });
    PublishResult = t.enum("PublishResult", {
      Success: t.unit(),
      Err: t.unit()
    });
    Published = t.object("Published", {
      owner: t.string(),
      module: t.string(),
      version: t.string()
    });
    SetResult = t.enum("SetResult", {
      Success: t.unit(),
      Err: t.unit()
    });
    SignupResult = t.enum("SignupResult", {
      Success: t.unit(),
      Err: t.unit()
    });
    Storage = t.object("Storage", {
      ownerKey: t.string(),
      value: t.string()
    });
  }
...
var init_change_password_procedure = __esm({
  "model/module_bindings/change_password_procedure.ts"() {
    "use strict";
    init_dist();
    init_types();
...
var params2, returnType2;
var init_publish_procedure = __esm({
  "model/module_bindings/publish_procedure.ts"() {
    "use strict";
    init_dist();
    init_types();
    ...
    returnType2 = PublishResult;
  }
});

// model/module_bindings/setitem_procedure.ts
var params3, returnType3;
var init_setitem_procedure = __esm({
  "model/module_bindings/setitem_procedure.ts"() {
    "use strict";
    init_dist();
    init_types();
    ...
    returnType3 = SetResult;
  }
});

// model/module_bindings/signup_procedure.ts
var params4, returnType4;
var init_signup_procedure = __esm({
  "model/module_bindings/signup_procedure.ts"() {
    "use strict";
    init_dist();
    init_types();
    ...
    returnType4 = SignupResult;
  }
});

// model/module_bindings/published_table.ts
var published_table_default;
var init_published_table = __esm({
  "model/module_bindings/published_table.ts"() {
    "use strict";
    init_dist();
    published_table_default = t.row({
      owner: t.string(),
      module: t.string(),
      version: t.string()
    });
...
var storage_table_default;
var init_storage_table = __esm({
  "model/module_bindings/storage_table.ts"() {
    "use strict";
    init_dist();
    storage_table_default = t.row({
...
var tablesSchema, reducersSchema, proceduresSchema, REMOTE_MODULE, tables, reducers2, SubscriptionBuilder, DbConnectionBuilder2, DbConnection;
var init_module_bindings = __esm({
  "model/module_bindings/index.ts"() {
    "use strict";
    init_dist();
    init_change_password_procedure();
    init_publish_procedure();
    init_setitem_procedure();
    init_signup_procedure();
    init_published_table();
    init_storage_table();
    tablesSchema = schema({
      published: table({
        name: "published",
    ...
    reducersSchema = reducers();
    proceduresSchema = procedures(
      procedureSchema("change_password", params, returnType),
      procedureSchema("publish", params2, returnType2),
      procedureSchema("setitem", params3, returnType3),
      procedureSchema("signup", params4, returnType4)
    );
    REMOTE_MODULE = {
      versionInfo: {
        cliVersion: "2.1.0"
    ...
    tables = makeQueryBuilder(tablesSchema.schemaType);
    reducers2 = convertToAccessorMap(reducersSchema.reducersType.reducers);
    SubscriptionBuilder = class extends SubscriptionBuilderImpl {
    };
    DbConnectionBuilder2 = class extends DbConnectionBuilder {
    };
    DbConnection = class _DbConnection extends DbConnectionImpl {
...
var OUT_LEN, BLOCK_LEN, CHUNK_LEN, CHUNK_START, CHUNK_END, PARENT, ROOT, IV, MSG_SCHEDULE, Output;
var init_hash = __esm({
  "model/hash.ts"() {
    "use strict";
    OUT_LEN = 32;
...
// model/db.ts
var storage, mkStore, LocalStored, noStore, rand, mkkey, User, randUser, RemoteDB;
var init_db = __esm({
  "model/db.ts"() {
    "use strict";
    ...
    init_pattern();
    init_hash();
    storage = typeof window !== "undefined" ? localStorage : /* @__PURE__ */ (() => {
      const db2 = /* @__PURE__ */ new Map();
      return {
        setItem: (key, value) => {
          db2.set(key, value);
        },
        getItem: (key) => db2.get(key) ?? null,
        clear: () => {
          db2.clear();
        }
      };
    })();
    mkStore = (request, persist, pattern) => {
      let listeners = [];
      let val = request();
      try {
    ...
    LocalStored = (key, pattern, defaultValue) => mkStore(() => JSON.parse(storage.getItem(key)) ?? defaultValue ?? fill(pattern), (v) => storage.setItem(key, JSON.stringify(v)), pattern);
    noStore = (pattern, defaultValue) => mkStore(() => defaultValue ?? fill(pattern), (v) => {
    }, pattern);
    rand = (digits) => Math.floor(Math.random() * 10 ** digits).toString().padStart(digits, "0");
    mkkey = (owner, key) => owner.replaceAll(":", "_:") + ":" + key;
    User = {
      userid: String,
      passhash: String
    };
    randUser = () => ({ userid: "u" + rand(4), passhash: hash(rand(6)) });
    RemoteDB = async () => new Promise((res, err) => {
      DbConnection.builder().withUri("wss://maincloud.spacetimedb.com/lexxtract").withDatabaseName("lexxtract").onConnect((c) => {
        ...
        const hot_cache = /* @__PURE__ */ new Map();
        const get = async (key, pattern, args = {}) => {
          let owner = args.owner || db2.userid;
          let owner_key = mkkey(owner, key);
          if (!hot_cache.has(owner_key)) {
            let value = args.upsertValue != void 0 ? null : await _get(owner, key).then((v) => v ?? args.defaultValue ?? null);
            hot_cache.set(owner_key, mkStore(
              () => value,
              async (v) => {
                db2.saving.update((x) => x + 1);
                console.log("saving: ", db2.saving.get(), owner, pwd());
          ...
          if (args.upsertValue != void 0) res2.set(args.upsertValue);
          return res2;
        };
        let db2 = {
          ...
          async changePassword(newPassword) {
            ...
            let res2 = await c.procedures.changePassword({ userid: db2.userid, passhash: pwd(), newPasshash: newhash });
            if (res2.tag != "Success") throw new Error("Failed to change password");
            signup({ userid: db2.userid, passhash: newhash });
          },
          get,
          publish: (owner, module, version, del) => c.procedures.publish({ owner: db2.userid, passhash: pwd(), module, version, del }).then((r) => {
        ...
        db2.signup(localUser.get()).then(() => res(db2)).catch(() => {
          db2.signup(randUser());
        });
...
var init_request = __esm({
  "controller/request.ts"() {
    ...
    init_db();
    init_json();
    localApiKey = LocalStored("openrouter_api_key", String, "");
  }
});

// controller/agent.ts
var cost_tracker, runningAgents;
var init_agent = __esm({
  "controller/agent.ts"() {
    ...
    init_functions();
    init_request();
    cost_tracker = LocalStored("cost_tracker", Number);
    runningAgents = LocalStored("running_agents", [String], []);
    runningAgents.set([]);
  }
});

// controller/functions.ts
var ProjectMission, TaxonomyExpert, ExtractionExpert, mkstarter, coordinatorAgent, Tools, functionReps;
var init_functions = __esm({
  "controller/functions.ts"() {
    ...
    init_agent();
    init_json();
    ProjectMission = `Lexxtract General Law Project
...
`;
    ...
    Tools = {
      ...
      addCategory: {
        ...
        runner: (taxonomy, _2, _3, _4, args) => {
          ...
          taxonomy.update((t2) => {
            if (!t2.categories[categoryName]) t2.categories[categoryName] = { description, subCategories: {} };
            return t2;
          });
          return "OK";
        }
      ...
      removeSubCategory: {
        ...
        runner: (taxonomy, _2, _3, _4, args) => {
          let { categoryName, subcategoryName } = args;
          taxonomy.update((t2) => {
            if (t2.categories[categoryName]) delete t2.categories[categoryName].subCategories[subcategoryName];
            return t2;
          });
          return "OK";
        }
    ...
    functionReps = Object.fromEntries(Object.entries(Tools).map(([k, t2]) => [k, t2.def]));
  }
});
...
__export(module_exports, {
  ...
  FunctionDefPattern: () => FunctionDefPattern,
  ModPathPattern: () => ModPathPattern,
  createModule: () => createModule,
  db: () => db
});
var CapabilityPattern, FunctionDefPattern, db, ModPathPattern, ExtractionPattern, createModule;
var init_module = __esm({
  async "controller/module.ts"() {
    "use strict";
    init_db();
    init_pattern();
    init_functions();
    CapabilityPattern = ["taxonomy", "documents", "prompt", "functions", "extraction", "agents"];
    FunctionDefPattern = {
      "description?": String,
    ...
    createModule = async (path2, tryCopy) => {
      let module_list = await db.get("modules", [ModPathPattern]);
      if (!module_list.get().map((x) => JSON.stringify(x)).includes(JSON.stringify(path2))) module_list.set([...module_list.get(), path2]);
      let modState = [];
      let mod_db = async (key, pattern, args = {}) => {
        let st = await db.get(path2.name + ":" + key, pattern, { owner: path2.owner, ...args });
        if (path2.owner != db.userid) st.set = async () => {
          tryCopy(
            () => createModule({ owner: db.userid, name: path2.name }, tryCopy).then((newmod) => Promise.all([
      ...
      const module = { db: mod_db, functions, taxonomy, extraction, documents, prompt: prompt2, path: path2, agents };
      if (path2.owner == db.userid) module.functions.set(functionReps);
      return module;
    };
  }
});
...
var loop = async () => {
  const { createModule: createModule2, db: db2 } = await init_module().then(() => module_exports);
  ...
  let mods = await db2.get_published();
  for (const mod of mods) {
    let modid = mod.owner + ":" + mod.module + ":" + mod.version;
    if (module_list.modules.some((l) => l.id == modid)) continue;
    db2.refresh();
    let mod_data = await createModule2({ owner: mod.owner, name: mod.module }, (c) => {
    });
